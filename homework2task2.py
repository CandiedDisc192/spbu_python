    ...
    with open(file_name, "r", encoding=file_encoding) as f_in:
        for line in f_in:
            ...
            for word in split_line:
                word_to_frequency[word] = word_to_frequency[word] + 1 if (word in word_to_frequency) else 1
...
def write_csv_word_frequency(file_name, dict_data, file_encoding="utf-8"):
    with open(file_name, "w", encoding=file_encoding) as csv_f_in:
        ...
        for word in dict_data:
            writer.writerow([word, dict_data[word]])


if __name__ == "__main__":
    user_txt_file_name = input("Введите путь до txt файла, в котором нужно  посчитать частоту встречаемости каждого"
                               " уникального слова: ")
    user_encoding = input("Введите кодировку этого файла: ")
    data = determine_word_frequency(user_txt_file_name, user_encoding)
    user_csv_file_name = input("Введите путь до пустого csv файла, в который будет записан результат (если файла нет, "
                               "то он будет создан): ")
    write_csv_word_frequency(user_csv_file_name, data, user_encoding)
