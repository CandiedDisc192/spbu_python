...
def get_angle_between_vectors(vector1, vector2):
    return degrees(acos(get_scalar_product(vector1, vector2) / (get_vector_length(vector1)
                                                                * get_vector_length(vector2))))
...
def get_matrices_sum(matrix1, matrix2, rows_count, columns_count):
    return [[matrix1[j][i] + matrix2[j][i] for i in range(columns_count)] for j in range(rows_count)]
...
def get_matrices_product(matrix1, matrix2, rows_count1, columns_count1, rows_count2, columns_count2):
    ...
    return [[get_scalar_product(matrix1[i], transposed_matrix2[j]) for j in range(columns_count2)] for i in
            range(rows_count1)]
...
def get_answer_from_vectors_menu(chosen_option):
    ...
    if chosen_option == "1":
        ...
        return "scalar product: {}".format(get_scalar_product(given_vector1, given_vector2))
    elif chosen_option == "3":
        given_vector2 = get_vector_from_user("second ")
        return "Angle between vectors: {} degrees".format(get_angle_between_vectors(given_vector1, given_vector2))


def get_answer_from_matrices_menu(chosen_option):
    matrix1_info = get_matrix_info_from_user("first ")
    if chosen_option == "1":
        transposed = get_transposed_matrix(matrix1_info[0], matrix1_info[1], matrix1_info[2])
        return transposed
    elif chosen_option == "2":
        ...
        matrix2_info = get_matrix_info_from_user("second ")
        product_matrix = get_matrices_product(matrix1_info[0], matrix2_info[0], matrix1_info[1],
                                              matrix1_info[2],
