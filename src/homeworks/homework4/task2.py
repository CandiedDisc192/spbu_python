...
def normalize(num: float):
    ...
    order = unsigned_bin.find(".") if unsigned_bin[0] != "0" else -len(unsigned_bin[2:unsigned_bin.find(mantissa[0])])
...
def to_fp(normalized: str, bits: int):
    ...
    sign, mantissa, original_order = int(normalized[0] == "-"), normalized[3:exp_i-2], int(normalized[exp_i+1:])
...
def main():
    ...
    user_choose = input("Available formats:\n1. FP64\n2. FP32\n3. FP16\nEnter selection(1-3): ")
    if user_choose == "1":
